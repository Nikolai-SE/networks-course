...
Тайвань: 35.229.225.152

![](2024-04-21-19-47-59.png)
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - Значением TTL: установлен равным 1 для первого запроса.

   ![](2024-04-21-19-49-51.png)
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - Информация о последних пакетам TCP и UDP, поле unused.

   ![](2024-04-21-19-52-56.png)
...
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - Значение TTL в вложенных пакетах отличается от 0 или 1.

   ![](2024-04-21-19-55-04.png)
   - Видимо из-за того, что пакеты были доставлены без истечения TTL. Эти сообщения являются сигналом для завершения работы traceroute.
...
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - Да, передача между маршрутизатора 16 и 17 заняла значительно больше времени.

   ![](2024-04-21-20-03-16.png)

   - Такое увеличение времени могло произойти во время перехода через длинный канал связи по дну моря.

   Однако оба адреса расположены в Техасе и принадлежат Google. https://www.speedguide.net/ip/142.251.226.156 
   Видимо Google фильтрует трафик или же еще что-то с ним делает.
